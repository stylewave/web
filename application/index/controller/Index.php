...
//seo优化+网站建设
    ...
    public function common(){
        ...
        $find = 'seo';
        $ci = strpos($request_uri, $find);
        if(!$ci){

            $where2['cateid'] = ["cateid",'in',"41, 72, 73, 74,75,76,77"];
            $casedata = Db::name('article')->where($where2)->order('id','desc')->limit('6')->select();
        
            $city = Db::name('city')->where(['cate'=>$id])->find();
        
            $where['cateid'] = ["cateid",'in',"45,47,48,68,95"];
            $where['cityid'] = $city['id'];
            $newsdata = Db::name('article')->where($where)->order(['orders'=>'asc','id'=>'desc'])->limit('9')->select();
        
        
            $where3['cateid'] = ["cateid",'in',"48"];
            $where3['cityid'] = $id;
            $newsdata2 = Db::name('article')->where($where3)->order(['orders'=>'asc','id'=>'desc'])->limit('9')->select();
