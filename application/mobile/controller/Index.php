...
class Index extends Bases
{
    public function index()
    {
        ...
        $banner = Db::name('banner')->where(['cateid'=>97,'ist'=>1])->order('orders','asc')->select();
        $about = Db::name('article')->where('cateid',57)->find();

        $where['cateid'] = ["cateid",'in',"41, 72, 73, 74,75,76,77"];
        $where['ist'] = 1;
        $case = Db::name('article')->where($where)->order(['orders'=>'asc','id'=>'desc'])->limit(6)->select();
        ...
        $newnav = Db::name('cate')->where(['ist'=>1,'pid'=>42])->order('orders','asc')->limit(4)->select();
      
        foreach($newnav as $k=>$v){
            $newnav[$k]['list'] = Db::name('article')->where('ist',1)->where('cateid',$v['id'])->order(['orders'=>'asc','id'=>'desc'])->limit(4)->select();
        }
...
//汇款方式
    ...
    public function case(){
        ...
        if(isset($pid)){
            $pid = $this->request->param('pid');
            $list = Db::name('article')->where('cateid',$pid)->order(['orders'=>'asc','id'=>'desc'])->paginate(8, false, ['query' => $this->request->param()]);
        }else{
           if(isset($title)) $where['titie'] = ['title','like','%'.$title.'%'];
            $where['cateid'] = ["cateid",'in',"41, 72, 73, 74,75,76,77"];
            $list = Db::name('article')->where($where)->order(['orders'=>'asc','id'=>'desc'])->paginate(8, false, ['query' => $this->request->param()]);
...
//        dump($data);die;
    ...
    public function news()
    {
        ...
        if(isset($pid)){
            $pid = $this->request->param('pid');
            $list = Db::name('article')->where('cateid',$pid)->order(['orders'=>'asc','id'=>'desc'])->paginate(6, false, ['query' => $this->request->param()]);
           ...
           if(isset($title)) $where['titie'] = ['title','like','%'.$title.'%'];
            ...
            $titledata = '全部分类';
            // $id = 45;
            $list = Db::name('article')->where($where)->order(['orders'=>'asc','id'=>'desc'])->paginate(6, false, ['query' => $this->request->param()]);
            // $list = Db::name('article')->where('cateid','in',[45, 46, 47, 48])->order('orders','desc')->paginate(8, false, ['query' => $this->request->param()]);
        }
   ...
   public function marketing_detail()
   {
    ...
    public function solution()
    {
        ...
        if(isset($title)) $where['titie'] = ['title','like','%'.$title.'%'];
        $where['cateid'] = 40;
        $list = Db::name('article')->where($where)->order(['orders'=>'asc','id'=>'desc'])->paginate(8);
        // dump($list);
